...
fs.writeFileSync(path.join(__dirname,'source_tables.json'),JSON.stringify(run('data'),null,2)+'\n');
const crypto=require('node:crypto');
const files=['dictionary/script.js','dictionary/embedded_data.js',...Object.keys(context.datasets).map(n=>'data/'+n+'.csv')];
fs.writeFileSync(path.join(__dirname,'manifest.json'),JSON.stringify({capturedAt:new Date().toISOString(), files:Object.fromEntries(files.map(file=>[file,crypto.createHash('sha256').update(fs.readFileSync(path.join(__dirname,'../..',file))).digest('hex')]))},null,2)+'\n');
